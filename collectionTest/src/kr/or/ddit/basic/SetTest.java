...
public class SetTest {
	 ...
	 */
		...
		// Set의 데이터를 수정하려면 수정작업을 처리하는 메서드가 따로 없기 때문에 해당 자료를 삭제한 후 추가해 주는 방식으로 처리한다.
...
//		System.out.println("clear 후 hs1 => " + hs1);
		...
		/*
		Set의 메서드는 순서(index)가 없기 때문에 List처럼 index를 이용해서 데이터를 하나씩 불러올 수 없다.
		...
		//      ==> Iterator에서 현재 데이터를 가르키는 포인터의 다음 번째 자리에 데이터가 있는지 검사한다
		//          데이터가 있으면 true, 없으면 false를 반환한다.
		while (it.hasNext()) {
			// Iterator의 next()메서드
			//      ==> Iterator의 포인터를 다음번째으로 이동시킨 후 이동한 자리의 데이터를 읽어와 반환한다
