...
/*
  	...
  	- TCP방식의 경우에는 스트림을 이용해서 송수신을 하고
  	  UDP방식의 경우에는 데이터그램을 이용해서 송수신 한다.
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				// 데이터를 수신하기  ==> receive()메서드 이용
				//	receive()메서드는 데이터가 올 때까지 기다린다.
				//	데이터가 도착하면 수신된 데이터에 패킷정보는 지정한 패킷객체변수에 저장된다.
				...
				// inpacket.getData() 
				//		==> 실제 읽어온 데이터를 byte배열로 반환
				// 		==> 수신을 패킷에 설정한 byte배열과 같다.
