...
public class CryptoUtill {
	...
	/**
	 * 매개변수로 받은 문자열을 SHA-512	방식으로 암호화하는 메서드
	 * (단방향 암호화 방식)
	 * 
	 * @param str 암호화할 문자열
	 * @return 암호화된 문자열
	...
	public static String encryptoAES256(String str, String key) throws UnsupportedEncodingException, NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, InvalidAlgorithmParameterException, IllegalBlockSizeException, BadPaddingException {
		/*
		 * 초기화 벡터(Initial Vector, IV) => 암호문이 패턴화 되지 않도록하는 데이터를 말한다.
		 ...
		 *         암호문 블록의 쌍이 발생하지 않도록 이전 단계의 암복호화한 결과가
		 *         현 단계에 영향을 주는 운영 모드를 말한다.
		 * - 패딩 : 마지막 블록이 블록의 길이와 항상 딱 맞아 떨어지지 않기 때문에 부족한 길이만큼을 '0'또는 임의의 비트로 채워 넣는 것을 말한다,
		 */
