...
public class ResponseTest02 extends HttpServlet {
	...
	- redirect 방식
		...
		==> 응답시 클라이언트의 웹브라우저에게 '이동할 URL'을 전송하면
			클라이언트의 웹브라우저가 해당 URL로 이동하는 방식이다.
			다른 페이지로 이동할 때는 GET방식으로 요청하기 때문에
			URL의 길이에 제한이 있다.
	...
	- 이동할 때 사용하는 메서드 : response객체의 sendRedirect()메서드이다.
		형식) sendRedirect("URL주소");
			...
			예) 이동할 주소가 "http://localhost/testWeb/test.do"라고
				하면 컨텍스트패스부터의 주소인 "/testWeb/test.do"와 같이 
				기술하면 된다.
	*/
		...
		// GET방식으로 보낼 데이터가 한글이 깨질 
		// 경우에 URLEncoding처리를 해주어야 한다.
		...
		response.sendRedirect(
			"/testWeb/responseRedirectTest.do?userName=" 
					+ name + "&tel=" + tel
		);
