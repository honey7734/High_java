...
public class CookieDeleteTest extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		- 저장된 쿠키정보 삭제하기
			...
			==> 쿠키의 수명은 쿠키를 저장하는 addCookie()메서드를 호출하기 전에 대당 쿠키객체의 setMaxAge()메서드를 이용하여 유지시간을 0으로 설정한다.
			형식) Cookie cookie변수 = new Cookie("쿠키변수", "쿠키값");
				 cookie변수.setMaxAge(시간);
				 	==> 시간이 0이면 쿠키가 바로 삭제되고, 시간이 음수이면 웹브라우저가 닫힐 때 쿠키가 삭제된다.
		 */
			...
			out.print("<h3>저장된 쿠키 데이터 삭제하기</h3>");
			if(cookieArr == null || cookieArr.length == 0) {
				...
				for(Cookie cookie : cookieArr) {
					...
					if("gender".equals(cookieName)) {
						// 해당 쿠키를 찾았으면 유지시간을 0으로 설정한 후 다시 저장한다.
						cookie.setMaxAge(0);
