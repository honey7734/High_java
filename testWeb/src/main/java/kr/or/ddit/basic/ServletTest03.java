...
2. 컨테이너는 web.xml에 정의된 url패턴을 확인하여 어느 서블릿을 통해 처리해야
   할지를 검색한다.
   (이 때 해당 서블릿이 로딩이 안된 경우에는 해당 서블릿을 로딩한다.
    처음 로딩시 init()메서드가 자동으로 호출된다.)
   (서블릿 버전 3.0이상에서는 애노테이션(@WebServlet)으로 설정할 수 있다.)
3. 서블릿 컨테이너는 요청을 처리할 개별 쓰레드 객체를 생성하여 해당 서블릿 객체의
   service()메서드르를 호출한다.
   (이 때 HttpServletRequest객체와 HttpServletResponse객체를 생성하여
    매개변수로 넘겨준다.)
4. service()메서드는 form의 method타입을 체크하여 적절한 메서드를 호출해 준다.
    (doGet, doPost, doPut, doDelete등...)
5. 요청 및 응답 처리가 완료되면 HttpServletRequest객체와 
