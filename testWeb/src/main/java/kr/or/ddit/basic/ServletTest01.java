...
// 서블릿이란? ==> 컨테이너(서블릿엔진)에 의해 관리되는 자바 기반의 웹 컴포넌트로서
...
/*
	...
	이 예제는 배포 서술자(web.xml)를 이용해서
	요청한 URL주소와 실행할 클래스를 연결해 주는 예제이다.
...
public class ServletTest01 extends HttpServlet {
	...
	// 이 영역에서는 대부분 service()메서드 또는 
	// doGet()메서드나 doPost()메서드를 재정의 해서 작성한다.
