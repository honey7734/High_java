...
/*
	...
	- 열거형의 속성 및 메서드
	  ...
	  2) ordinal()  ==> 열거형 상수가 정의 된 순서값(index값)을 반환한다.
	  					(0부터 시작)
	  3) valueOf("열거형상수형")	==> 지정된 열거형에서 '열거형상수명'과 일치하는 열거형 상수를 반환
	...
	방법2)  ==> 상수에 임의의 값을 지정하는 방법 
		enum 열거형이름 {
			...
			// 열거형의 생성자를 만든다.
			// 열거형의 생성자는 열거형 상수에 저장된 '값들'을 값들이 저장될 변수에 셋팅하는 역할을 한다
			// 열거형 생성자의 접근제한자는 묵시적으로 private이다.
			
			// 변수형의 개수는 '값들'의 개수와 같아야 하고 각 변수들의 자료형도 '값들'의 자료형과 같아야 한다.
...
public class EnumTest {
	...
	public static void main(String[] args) {
		...
		if(ConstTest.RED == ConstTest.TWO) {  //의미적으로 잘못된것이지만 문법적으로 문제가 없음
		...
		*/
		...
		switch (mycnt) {		  
		// case문에 지정하는 열거형값은 열거형이름을 뺀 상수명만 사용해야한다
			case ONE: 
